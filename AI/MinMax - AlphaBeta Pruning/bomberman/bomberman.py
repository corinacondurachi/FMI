...
class Joc:
    ...
    def final(self, jucator):
        # returnam simbolul jucatorului castigator daca nu mai exista mutari (jucatorul s-a blocat) sau a ramas fara vieti
        # sau returnam 'remiza'
    ...
    def pozitie_caracter(self, ch):
        # folosita pentru a gasi bombele si pozitia personajului pe harta
    ...
    def explozie(self, pozitie):
        # explozia va afecta ambii jucatori (se va scadea o viata de la amandoi)
        # daca bomba intalneste o alta bomba declanseaza alta explozie
        # functia scade cate o viata de la fiecare jucator prins in explozie
    ...
    def muta_jucator(self, jucator, directie, foloseste_bomba):
        ...
        if self.matr[poz_noua[0]][poz_noua[1]] not in [' ', 'p']:  # mutarea nu este valida
            if poz_noua == poz_bomba:  # daca jucatorul e blocat de propria bomba, singura varianta e sa o declanseze si sa
                                       # mearga acolo
                if foloseste_bomba: # daca alege sa foloseasca bomba pornim explozia
                    if self.explozie(poz_bomba): # daca dupa ce a folosit bomba, unul dintre jucatori a ramas fara vieti, ne oprim
                        return True
                    if self.matr[poz_juc[0]][poz_juc[1]] == jucator: # daca jocul continua
        ...
        if foloseste_bomba:
            if poz_bomba is None:  # daca nu am pus bomba, o putem acum
                ...
                if self.explozie(poz_bomba):  # daca a murit cineva, nu mai continuam
                    return True               
                if self.matr[poz_juc[0]][poz_juc[1]] == jucator: # daca nu a murit nimeni
                    self.matr[poz_juc[0]][poz_juc[1]] = ' '
    ...
    def mutari(self, jucator):
        ...
        for directie in directii:
            ...
            # in total sunt 4 (directiile) * 2 (foloseste bomba/ nu foloseste bomba) = 8 mutari posibile
    ...
    def scor(self, jucator):
        nr = 0
        if jucator == self.JMIN:  # vietile in plus sunt foarte importante intrucat un joc se
                                  # termina atunci cand un jucator ramane fara vieti
            nr += 1000 * self.vieti_jmin
        else:
            nr += 1000 * self.vieti_jmax

        bomba = self.BMAX if jucator == self.JMAX else self.BMIN  # la scor adaugam 33 (nr maxim de
                                                                  # casute pe care le poate acoperi bomba plasata)
        poz_bomba = self.pozitie_caracter(bomba)  
    ...
    def fct_euristica(self):
        return self.scor(Joc.JMAX) - self.scor(Joc.JMIN)
...
def main():
    ...
    raspuns_valid = False
    while not raspuns_valid:
        n = input("Adancime maxima a arborelui: (1-5, 1 fiind cel mai usor, iar 5 cel mai dificil)")
        if n.isdigit():
    ...
    # preiau timpul in milisecunde la inceputul jocului
    t_inc_joc = int(round(time.time() * 1000))
    ...
    print("Intructiuni:\n"
          ...
          "Daca cealalta bomba este atinsa de o explozie, explodeaza si aceasta. \n"
          "Scrieti 'exit' si apasati 'Enter' pentru a iesi. \n")
        ...
        if stare_curenta.j_curent == Joc.JMIN:
            ...
            while not raspuns_valid:
                try:
                    ...
                    if user_input not in [' w', ' a', ' s', ' d', 'w', 'a', 's', 'd','exit']:
    ...
    # preiau timpul in milisecunde de la sf jocului
    t_sf_joc = int(round(time.time() * 1000))
    print("Jocul a durat " + str(t_sf_joc - t_inc_joc) + " milisecunde. \n")
    print("Jucatorul a mutat de", mutari_jucator , "ori, iar calculatorul de" , mutari_calculator , "ori.")
