...
class Nod:
    def __init__(self, info, h = None):
        self.info = info
        # h= media aritmetica dintre nr canibalilor si nr misionarilor care nu au fost transportati inca
        if h is not None:
            self.h = h
        else:
...
class Problema:
    def __init__(self):
        ...
        self.N = 3 #nr misionari, respectiv canibali
...
class NodParcurgere:
    ...
    def expandeaza(self):
        ...
        # m- nr de misionari; pt fiecare m luam fiecare nr de canibali <=m atata timp cat avem loc in barca
        # daca m=0, atunci luam canibali<=M si vedem daca putem traversa
        for m in range(min(info[mal][0], self.problema.M)+1):
            if m == 0:
                c_max = self.problema.M
            else:
                #nr maxim de canibali pe care ii putem lua este minimul dintre locurile ramse in barca, nr misionarilor
                # (nu poate depasi nr misionarilor) si nr canibalilor de pe malul repsectiv
